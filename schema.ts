...
export const schemaType = <V extends TupleMapBuilderResult>(value: V): InferTupledMap<V> => value as any;
...
type ForEachMapValuePrependKey<T, K extends string> = T extends [infer F, ...infer R]
  ? //   ? [PrependKeyToField<F, K>, ...ForEachMapValuePrependKey<R, K>]
    F extends [infer FK, infer FV]
    ? FV extends MapAttribute<[...TupleKeyValuePeer<string, unknown>[]]>
      ? ForEachMapValuePrependKey<InferAttributeValue<FV>, `${K & string}.${FK & string}`>
      : [[`${K & string}.${FK & string}`, FV], ...ForEachMapValuePrependKey<R, K>]
    : never
  : //   [
    //       F extends [infer FK, infer FV]
    //         ? FV extends object
...
//   ? F extends [infer FK, infer FV]
//     ? FV extends object
//       ? FV extends [infer FFK, infer FFV]
//         ? [[`${K & string}.${FK & string}.${FFK & string}`, FFV], ...ForEachMapValuePrependKey<R, K>]
//         : never
//       : [[`${K & string}.${FK & string}`, FV], ...ForEachMapValuePrependKey<R, K>]
//     : never
//   : T;
...
type F = ForEachMapValuePrependKey<
  ...
  [["field1", number], ["field2", string], ["field3", MapAttribute<[["nested_field1", string]]>]],
  "map"
>;
